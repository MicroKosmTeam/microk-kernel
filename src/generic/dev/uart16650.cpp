...
static inline int IsTransmitEmpty(UART::UARTDevice *device) {
#if defined(ARCH_x64)
...
static inline int SerialReceived(UART::UARTDevice *device) {
#if defined(ARCH_x64)
...
Device *CreateUARTDevice() {
	UART::UARTDevice *device = new UART::UARTDevice;
...
int InitializeDevice(Device *device, va_list ap) {
	UART::UARTDevice *uartDevice = (UART::UARTDevice*)device;
	...
	uartDevice->PutChar = PutChar;
	uartDevice->PutString = PutString;
	uartDevice->GetChar = GetChar;
